...
class NotificationServicesTests(TestCase):
    ...
    def _create_borrowing(self, days_overdue: int = 0, returned: bool = False) -> Borrowing:
    ...
    def test_get_admin_chat_ids_parsing(self):
        with patch.dict(os.environ, {"TELEGRAM_ADMINS_CHAT_IDS": "123, 456 , , abc,789"}, clear=False):
    ...
    def test_get_admin_chat_ids_empty_when_missing(self):
        with patch.dict(os.environ, {"TELEGRAM_ADMINS_CHAT_IDS": "", "TELEGRAM_ADMINS_CHAT_ID": ""}, clear=False):
            ids = notif_services._get_admin_chat_ids()
    ...
    def test_notify_overdue_borrowings_admin_skips_without_admin_ids(self, mock_send):
        self._create_borrowing(days_overdue=2)
        with patch.dict(os.environ, {"TELEGRAM_ADMINS_CHAT_IDS": ""}, clear=False):
            count = notif_services.notify_overdue_borrowings_admin()
        self.assertEqual(count, 1)
        mock_send.assert_not_called()

    @patch.object(notif_services, "_sync_send_messages")
    def test_notify_overdue_borrowings_admin_sends_with_admin_ids(self, mock_send):
        self._create_borrowing(days_overdue=3)
        with patch.dict(os.environ, {"TELEGRAM_ADMINS_CHAT_IDS": "1,2"}, clear=False):
            count = notif_services.notify_overdue_borrowings_admin()
        self.assertEqual(count, 1)
    ...
    def test_notify_payment_success_admin_only_for_paid(self, mock_send):
        ...
        p_pending = self._create_payment(borrowing, status=Payment.Status.pending)
        with patch.dict(os.environ, {"TELEGRAM_ADMINS_CHAT_IDS": "100"}, clear=False):
            notif_services.notify_payment_success_admin(p_pending.id)
        mock_send.assert_not_called()

        p_paid = self._create_payment(borrowing, status=Payment.Status.paid, money=Decimal("12.34"))
        with patch.dict(os.environ, {"TELEGRAM_ADMINS_CHAT_IDS": "100"}, clear=False):
